function isInvalidInputField(fieldData, field) {
  switch (typeof fieldData) {
    case 'string':
      ...
      if (field === 'creator_email' && !isEmptyField) {
...
export function getValidationWithField(meal) {
  const errorFieldsStack = [];
  for (const fieldKey in meal) {
    const isDataIncompleteOrInvalid = isInvalidInputField(meal[fieldKey], fieldKey);
    if (isDataIncompleteOrInvalid) {
      const field = fieldKey?.includes('creator')
        ? fieldKey?.includes('email')
          ? 'email'
          : 'name'
        : fieldKey;
      errorFieldsStack.push(field);
    }
  }
  console.log(errorFieldsStack);
  return errorFieldsStack;
}
